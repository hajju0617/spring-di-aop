...
public class UserDaoImpl implements UserDao {
    ...
    public int deleteUser(String id) {
        ...
        // Connection, PreparedStatement는 사용했으면 close로 닫아줘야함.
        // close() 해주지 않으면 메모리가 제대로 반환되지 않아서 점점 메모리가 부족해질 수 있어서 문제가 발생할 가능성이 있음.
...
//            try { if(conn!=null)  conn.close();  } catch (SQLException e) { e.printStackTrace();}
    ...
    public User selectUser(String id) {
        ...
        try {
            conn = ds.getConnection();
            pstmt = conn.prepareStatement(sql); // SQL Injection공격 방지, 성능향상
            pstmt.setString(1, id);
...
//            try { if(pstmt!=null) pstmt.close(); } catch (SQLException e) { e.printStackTrace();}
//            try { if(conn!=null)  conn.close();  } catch (SQLException e) { e.printStackTrace();}
            close(rs, pstmt, conn);  //     private void close(AutoCloseable... acs) 라는 메서드를 사용해서 close처리했음. {
        }
...
//        PreparedStatement pstmt = null;
        ...
        try (
                ...
                PreparedStatement pstmt = conn.prepareStatement(sql); // SQL Injection공격 방지, 성능향상
        ){
