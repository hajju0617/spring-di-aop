...
public class HomeController {
	@Autowired
	WebApplicationContext servletAC; // Servlet AC (Servlet ApplicationContext는 이렇게 주입 받을 수 있음.)

	@RequestMapping(value = "/", method = RequestMethod.GET)
	public String home(Locale locale, HttpServletRequest request, Model model) {
		// 원래는 서블릿에서는 request.getServletContext()하면 되지만, 컨트롤러는 HttpServlet을 상속받지 않아서 아래와 같이 해야함.
